...
export default function AdminUsersPage() {
  ...
  const approveUser = async () => {
    ...
    try {
      const response = await fetch("/api/admin/users", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: selectedUser.id, action: "approve" }) });
      if (!response.ok) { const data = await response.json(); throw new Error(data.error ?? "Unable to approve clinician."); }
      const approved = { ...selectedUser, adminApproved: true, verified: true };
      setSelectedUser(approved); setUsers((current) => current.map((user) => user.id === approved.id ? approved : user));
    } catch (approvalError) { setError(approvalError instanceof Error ? approvalError.message : "Unable to approve clinician."); } finally { setSaving(false); }
  };
