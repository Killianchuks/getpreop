...
function VerifyEmailForm() {
  ...
  const plan = searchParams.get("plan") ?? "per-case";
  ...
  async function verify(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (role === "SURGERY_CENTER" || role === "PATIENT") router.push(`/checkout?plan=${encodeURIComponent(plan)}&billing=${encodeURIComponent(billing)}&email=${encodeURIComponent(email)}`);
    else router.push(role === "ANESTHESIOLOGIST" ? "/anesthesiologists/workspace" : "/login");
  }
